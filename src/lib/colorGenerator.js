...
/**@typedef {import('../themes/semantic-rainbow.json')['themes'][number]} ColorConfig */
...
const generateColors = (tinycolor, config) => {
  ...
  const numColors = (baseNumber * modNumber) +
    (baseNumber * extraCombinationsNumber);
  ...
  const manualColors = (() => {
    ...
    for (const f in config.modifications) {
      ...
      for (const k in filter) {
        (Object.hasOwnProperty.call(filter, k) && k !== "default") &&
          manualFilters++;
      }
    }
  ...
  const meta = {
    ...
    baseNumber,
    modNumber,
    extraCombinationsNumber,
    manualPercent: ((manualColors / numColors) * 100).toFixed(2),
  };
  /**
  ...
  const applyColors = (base, variations, lang) => {
    let color = new tinycolor(
      (lang && config.baseTokenColors[`${base}:${lang}`]) ||
        config.baseTokenColors[base],
    );
    variations.forEach((v) => {
      const def = (lang && config.modifications[v][`${base}:${lang}`]) ||
        config.modifications[v][base] ||
        (lang && config.modifications[v][`default:${lang}`]) ||
        config.modifications[v].default;
      for (const k in def) {
        (Object.hasOwnProperty.call(def, k)) &&
          (k in specialModifications
            ? (color = specialModifications[k](color, def[k]))
            : color[k](def[k]));
      }
    });
    return color;
  ...
  const encode = (color, text, lang) => {
    const finalColor = color
      [`toHex${color.getAlpha() === 1 ? "" : "8"}String`]();
    ...
    if (config.alias) {
      for (const k in config.alias) {
        (Object.hasOwnProperty.call(config.alias, k) &&
          (new RegExp(`\\b${k}\\b`, "gm")).test(finalText)) &&
          config.alias[k].forEach((a) =>
            semanticRules[finalText.replace(new RegExp(`\\b${k}\\b`, "g"), a)] =
              typeof rule === "string"
                ? { foreground: rule, alias: true }
                : { ...rule, alias: true }
          );
      }
    }
    //Saving the TextMate fallback rule
    if (config.fallbacks && config.fallbacks[finalText]) {
      fallBackRules.push({
        name: finalText,
        scope: config.fallbacks[finalText],
        settings: textMateTransform(rule),
      });
    } else if (
      config.fallbacks && noLangText !== finalText &&
      config.fallbacks[noLangText]
    ) {
      fallBackRules.push({
        name: finalText,
        scope: config.fallbacks[noLangText].map((t) => `source.${lang} ${t}`),
        settings: textMateTransform(rule),
      });
    }
  };
  ...
  for (const t in config.baseTokenColors) {
    ...
    for (const lang of new Set([lg].concat(config.defaultLanguages || []))) {
      ...
      for (
        const [c, l] of config.modifierCombinations.map((c) => c.split(":"))
          .map(([c, l]) => [c.split("."), l])
      ) {
 ...
 */
const isColorVar = (values) => (name) =>
  (!name.startsWith("#")) && name.includes(".") && name in values;
...
const resolveColors = (dynamicValues, staticValues) => {
  ...
  dynamicValues.tokenColors && dynamicValues.tokenColors.forEach((t) => {
    if (cv(t.settings.foreground)) {
      t.settings.foreground = staticValues[t.settings.foreground];
    }
  });

  for (const k in dynamicValues?.semanticTokenColors ?? {}) {
    if (
      Object.hasOwnProperty.call(dynamicValues.semanticTokenColors, k) &&
      dynamicValues.semanticTokenColors
    ) {
      const element = dynamicValues.semanticTokenColors[k];
      if (typeof element === "string") {
        if (
          cv(element) && dynamicValues.semanticTokenColors &&
          dynamicValues.semanticTokenColors[k]
        ) dynamicValues.semanticTokenColors[k] = staticValues[element];
      } else if (element && cv(element.foreground)) {
        element.foreground = staticValues[element.foreground];
      }
    }
  }
};
